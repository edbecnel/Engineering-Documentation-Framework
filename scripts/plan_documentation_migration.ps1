...
function Get-RelativePath {
    ...
    $relativePath = $FullPath.Substring($ProjectRootFull.Length).TrimStart(
        [System.IO.Path]::DirectorySeparatorChar,
        [System.IO.Path]::AltDirectorySeparatorChar
    )
...
function Test-CanonicalMarkdownLocation {
    ...
    return (
        ...
        $RelativePath -eq "AI_WORKFLOW.md" -or
...
function Get-Recommendation {
    ...
    if ($lower -match "ai|prompt|cursor|continue|ollama|claude|gpt|copilot") {
    ...
    return @{ Destination = "docs/Reference/$fileName"; Reason = "No strong filename match found; review manually. Reference is the safest default recommendation." }
}

$ProjectName = Split-Path -Path $ProjectRootFull -Leaf
$Candidates = New-Object System.Collections.Generic.List[string]

Get-ChildItem -LiteralPath $ProjectRootFull -Recurse -File -Include *.md, *.markdown | Sort-Object FullName | ForEach-Object {
...
Write-Host "2. Move only documents whose purpose is clear."
Write-Host "3. Add links from README.md and PROJECT_INDEX.md where appropriate."
Write-Host "4. Archive obsolete or superseded documents with a short migration note."
Write-Host "5. Re-run the analysis and migration scripts after each documentation cleanup pass."
